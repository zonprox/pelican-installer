...
trap 'say_err "Fail at: ${BASH_COMMAND} (exit $?)"; exit 1' ERR
...
ensure_sury(){
  [[ -f /etc/apt/sources.list.d/sury-php.list ]] || {
...
ensure_php_84(){
  ensure_sury
  apt-get install -y php8.4 php8.4-fpm php8.4-cli php8.4-mbstring php8.4-xml php8.4-curl php8.4-zip php8.4-gd php8.4-bcmath php8.4-mysql php8.4-redis
...
ensure_docker(){
  ...
  say_info "Installing Docker CE (official repo)…"
  install_base
...
# Ports
port_busy(){ ss -ltnp | grep -qE "LISTEN\s+.*:$1(\s|$)"; }
open_port_ufw(){ command -v ufw >/dev/null 2>&1 && ufw allow "$1"/tcp || true; }

# Nginx real-ip for Cloudflare
...
cf_upsert_a_record(){
  ...
  res="$(mktemp)"
  if [[ -n "$rec_id" ]]; then
    http="$(curl -sS -o "$res" -w '%{http_code}' -X PUT "${hdr[@]}" --data "{\"type\":\"A\",\"name\":\"${name}\",\"content\":\"${content}\",\"ttl\":120,\"proxied\":${proxied}}" "${base}/${rec_id}")" || http=000
  else
    http="$(curl -sS -o "$res" -w '%{http_code}' -X POST "${hdr[@]}" --data "{\"type\":\"A\",\"name\":\"${name}\",\"content\":\"${content}\",\"ttl\":120,\"proxied\":${proxied}}" "${base}")" || http=000
  fi
  if [[ "$http" != "200" && "$http" != "201" ]]; then
    say_warn "Cloudflare upsert failed (HTTP $http): $(cat "$res")"
    rm -f "$res"; return 1
  fi
  rm -f "$res"; say_ok "Cloudflare DNS: ${name} → ${content} (proxied=${proxied})"
}
