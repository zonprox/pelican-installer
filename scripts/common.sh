...
ensure_php_84(){
  ...
  apt-get install -y \
    ...
    php8.4-mysql php8.4-redis php8.4-intl php8.4-sqlite3
  systemctl enable --now php8.4-fpm
}

# Check if a PHP extension is loaded (CLI)
php_has_ext(){
  local ext="$(echo "$1" | tr '[:upper:]' '[:lower:]')"
  php -m 2>/dev/null | tr '[:upper:]' '[:lower:]' | sed 's/ //g' | grep -qx "$ext"
}

# Map extension name -> apt package
php_ext_to_pkg(){
  local ext="$1"
  case "$ext" in
    intl)         echo "php8.4-intl" ;;
    pdo_sqlite|sqlite3) echo "php8.4-sqlite3" ;;
    pdo_mysql|mysql)    echo "php8.4-mysql" ;;
    redis)        echo "php8.4-redis" ;;
    gd)           echo "php8.4-gd" ;;
    bcmath)       echo "php8.4-bcmath" ;;
    curl)         echo "php8.4-curl" ;;
    zip)          echo "php8.4-zip" ;;
    mbstring)     echo "php8.4-mbstring" ;;
    xml)          echo "php8.4-xml" ;;
    imagick)      echo "php-imagick" ;; # ngoài luồng PHP phiên bản
    *)            echo "" ;;
  esac
}

# Ensure a list of PHP extensions is installed (idempotent)
ensure_php_exts(){
  local need_pkgs=() ext pkg changed=0
  for ext in "$@"; do
    if php_has_ext "$ext"; then
      say_info "PHP extension already loaded: $ext"
    else
      pkg="$(php_ext_to_pkg "$ext")"
      if [[ -n "$pkg" ]]; then
        need_pkgs+=("$pkg")
      else
        say_warn "Unknown PHP extension mapping: $ext (skipped)"
      fi
    fi
  done
  if ((${#need_pkgs[@]})); then
    say_info "Installing missing PHP extensions: ${need_pkgs[*]}"
    apt-get install -y "${need_pkgs[@]}"
    changed=1
  fi
  if ((changed)); then
    systemctl restart php8.4-fpm || true
    say_ok "PHP-FPM restarted after installing extensions."
  fi
}
...
cf_preflight_warn(){
  local http
  if [[ "${CF_AUTH:-token}" == "global" ]]; then
    http="$(curl -sS -o /dev/null -w '%{http_code}' -H "X-Auth-Email: ${CF_API_EMAIL}" -H "X-Auth-Key: ${CF_GLOBAL_API_KEY}" -H "Content-Type: application/json" "https://api.cloudflare.com/client/v4/zones/${CF_ZONE_ID}")"
  else
    http="$(curl -sS -o /dev/null -w '%{http_code}' -H "Authorization: Bearer ${CF_API_TOKEN}" -H "Content-Type: application/json" "https://api.cloudflare.com/client/v4/zones/${CF_ZONE_ID}")"
  fi
  [[ "$http" == "200" ]] || { say_warn "Cloudflare preflight failed (HTTP $http)"; return 1; }
  return 0
}
...
cf_upsert_a_record(){
  ...
  local res http rec_id; res="$(mktemp)"
  http="$(curl -sS -o "$res" -w '%{http_code}' "${hdr[@]}" "${base}?type=A&name=${rec_name}")" || http=000
  [[ "$http" == "200" ]] && rec_id="$(jq -r '.result[0].id // empty' "$res")"
  rm -f "$res"
  res="$(mktemp)"
  if [[ -n "${rec_id:-}" ]]; then
    http="$(curl -sS -o "$res" -w '%{http_code}' -X PUT "${hdr[@]}" --data "{\"type\":\"A\",\"name\":\"${rec_name}\",\"content\":\"${content}\",\"ttl\":120,\"proxied\":${proxied}}" "${base}/${rec_id}")" || http=000
  else
    http="$(curl -sS -o "$res" -w '%{http_code}' -X POST "${hdr[@]}" --data "{\"type\":\"A\",\"name\":\"${rec_name}\",\"content\":\"${content}\",\"ttl\":120,\"proxied\":${proxied}}" "${base}")" || http=000
  fi
